...
- [ ] All plain wallet address storage paths use `HMAC-SHA256(SERVER_SALT, ...)` before insert — no new columns store raw addresses in `listings`, `responses`, `chat_rooms`, `sessions`, `reputation`, `invoices`
...
- [ ] All required environment variables are set in the production environment:
  ...
  - `DEV_MODE=false`
- [ ] `WALLET_ENC_KEY` is different from `SERVER_SALT` (they serve different functions; reusing them is a mistake)
