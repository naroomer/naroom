# Translation Task — Georgian (ქართული)

## Instructions

Translate the text below from English to Georgian.

**Rules:**
- Do NOT translate: NA Room, BTC, LTC, $5, $15, $150, $1,000, Tor, .onion, Tbilisi, Batumi, Buenos Aires, São Paulo, Almaty, Yerevan, Moscow, Nha Trang, Da Nang
- Keep emoji exactly as-is (👍, 👎, ⚠)
- Keep number formatting exactly as-is ($150, $1,000, 24, 30)
- Translate #1 / #2 / #3 as-is (keep the # symbol)
- "peer" = თანატოლი (verified term)
- "peer support" = თანატოლური მხარდაჭერა
- "session" = სესია
- "listing" = განცხადება
- "board" = დაფა
- "wallet" = საფულე
...
*(Title already translated — keep as-is)*
...
*(Translate the lines below. Keep the arrows ↓ and the code block formatting.)*
...
Limit: the number of active responses you can have open at once equals your balance divided by $1,000. For example:
- $1,000 balance → 1 active response
- $2,000 balance → 2 active responses
- $3,000 balance → 3 active responses
...
## Glossary of verified terms (use consistently)
...
| balance | ბალანსი |
| anonymous | ანონიმური |
| end-to-end encrypted | ბოლოდან ბოლომდე დაშიფრული |
| urgent | გადაუდებელი |
| can wait | შეიძლება დაელოდოს |
| expired | ვადა გავიდა |
| withdrawal support | აბსტინენციის დროს მხარდაჭერა |
| relapse prevention | რეციდივის პრევენცია |
| recovery plan | გამოჯანმრთელების გეგმა |
| crisis support | კრიზისული მხარდაჭერა |
| motivation | მოტივაცია |
| just talk | უბრალოდ საუბარი |
| feedback | შეფასება / გამოხმაურება |
| reputation | რეპუტაცია |
| account | ანგარიში |
| slot | სლოტი |
| invoice | ინვოისი |
| confirm / confirmation | დადასტურება |
| emergency services | საგანგებო სამსახურები |
