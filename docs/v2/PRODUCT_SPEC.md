...
- 2026-07-26: Task 11 — Helper purchase journey hardened end-to-end. Changes:
  ...
  (5) City registry unified: single `AllCities` Go registry; listing validation, country lock, and Informer all use this registry; frontend gets the 13 enabled destinations from `GET /v2/board/cities`. Bangkok, Chiang Mai, Phuket, Istanbul, Dubai, Lisbon, Valencia, and Malaga remain disabled registry records and are unavailable in every V2 flow.
  ...
  (10) Board discovery: honest `active_count` (sample cards excluded); exactly 3 sample cards per enabled city, clearly marked as examples. A sample opens the real listing UI through the wallet form, but the final action is intercepted before any API mutation: no balance lookup, purchase token, invoice, QR, contact, storage entry, or DB write is created. The user sees a localized sample notice and returns to the board. The board keeps an automatic empty state when `active_count=0`; the selector contains 13 enabled cities; reputation shows "today"/"both counters"/"Platform nickname" label.
